...
// بنبني الاتصال بـ Gemini بشكل آمن: لو مفيش مفتاح API متظبط في هذا الـ deployment،
// التطبيق مش هيكرش أو يظهر شاشة بيضا — بس ميزات الذكاء الاصطناعي هترجع نتيجة فاضية،
// وباقي أجزاء التطبيق (اللي مالهاش علاقة بالـ AI) هتفضل شغالة عادي.
function getAIClient(): GoogleGenAI | null {
  try {
    ...
    console.error('Gemini AI client unavailable:', e);
    return null;
  }
}
...
export const fetchEgyptianHolidays = async (year: string, language: Language): Promise<Holiday[]> => {
  ...
  try {
    ...
    console.error("Failed to fetch/parse holidays:", e);
    return [];
  }
};
